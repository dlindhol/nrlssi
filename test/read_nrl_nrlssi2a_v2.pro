function read_nrl_nrlssi2a_v2,infile,starty,endy
  ...
  startyear = starty
  endyear = endy
  ndy = endyear+1-startyear ;for yearly averaged values
...
;  dn=fltarr(ndy)
  ...
  tspec=fltarr(ndy)
  wave=fltarr(3785)
  dat0=fltarr(2);for year and NRLTSI2 in W/m2
...
;    dn(n)=julday(dat0(1),dat0(2),dat0(0))-julday(1,1,startyear)+1
    ti(n)=dat0(1)     ; calculated directly from TSI model
    readf,1,dat
