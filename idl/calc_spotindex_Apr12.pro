...
; - the dependence of contrast on area is included
;
...
;
close,5
openr,5,'/home/lean/data/solaractivity/spots/betasun.dat'
beta=fltarr(366)
betad=fltarr(366)
;
dumi='    '
...
if(iyear lt 2000) then $
       infile='~/data/ngdc/sunspot_regions/USAF/usaf_mwl.'+string(yy,'$(i2)')
if((iyear ge 2000) and (iyear le 2004)) then $
       infile='~/data/ngdc/sunspot_regions/USAF/usaf_mwl.0'+string(yy,'$(i1)')
if((iyear ge 2005) and (iyear lt 2010)) then $
       infile='~/data/ngdc/sunspot_regions/USAF/USAF.0'+string(yy,'$(i1)')
if(iyear eq 2010) then $
       infile='~/data/ngdc/sunspot_regions/USAF/USAF.'+string(yy,'$(i2)')
...
if(iyear eq 2010) then $
       infile='~/data/ngdc/sunspot_regions/USAF/USAF_Denig_2010-2012.txt'
...
;
excess=0.3235
rsun=6.9599E10  ;not used
au=1.495979E13  ;not used
...
alat=fltarr(170,ndays)  ;not used as 2D array
...
readf,1,dumi
; print,dumi
IC=strmid(dumi,0,2) ;not used
;might as well read 2,6 since it is reconstructed as mdate below
my=fix(strmid(dumi,2,2))
mm=fix(strmid(dumi,4,2))
md=fix(strmid(dumi,6,2))
mut=strmid(dumi,9,4)    ;not used
...
along=float(along)
wils=strmid(dumi,20,4)  ;not used
fld=strmid(dumi,25,1)   ;not used
mwil=strmid(dumi,27,5)  ;not used
smw=strmid(dumi,32,1)   ;not used
noaa=strmid(dumi,33,5)
snoa=strmid(dumi,38,1)  ;not used
zc=strmid(dumi,39,1)    ;not used
pc=strmid(dumi,40,1)    ;not used
cmpct=strmid(dumi,41,1) ;not used
numspot=strmid(dumi,43,2) ;not used
; check for no spots
if(numspot eq '  ') then numspot=' 0'
numspot=fix(numspot)
longext=strmid(dumi,46,2) ;not used
...
IMPY=strmid(dumi,53,2)  ;not used
IMPM=strmid(dumi,55,2)  ;not used
CIMPD=strmid(dumi,57,4)  ;not used
MPY=strmid(dumi,62,2)  ;not used
MPM=strmid(dumi,64,2)  ;not used
CMPD=strmid(dumi,66,4)  ;not used
ISER=strmid(dumi,71,3)  ;not used
AQU=strmid(dumi,75,1)  ;not used
...
; print,i,mdate,idate(i)
    ...
    num=numobs(i)
    area(num-1,i)=iarea
    alat(num-1,i)=xlat ;not used as 2D array
...
; note that this quantity is not given in the 2011 file from Denig
    ...
    spotnum(num-1,i)=numspot ;not used
    istn(num-1,i)=js
...
;
goto, cont1000
cont702: print,'Finished reading file ..last date was ',mdate
;     XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
;
